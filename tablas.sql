...
CREATE TABLE EMPLEADO (
    ...
    CONSTRAINT fk_empleado_sucursal FOREIGN KEY (id_scrsal) REFERENCES SUCURSAL(id_scrsal)
);
...
CREATE TABLE MESA (
    ...
    CONSTRAINT fk_mesa_sucursal FOREIGN KEY (id_scrsal) REFERENCES SUCURSAL(id_scrsal)
);
...
CREATE TABLE PROVEEDOR (
    ...
    email VARCHAR(50) UNIQUE,
    id_scrsal INT NOT NULL,
    CONSTRAINT fk_proveedor_sucursal FOREIGN KEY (id_scrsal) REFERENCES SUCURSAL(id_scrsal)
);
...
CREATE TABLE COMPRA (
    ...
    id_scrsal INT NOT NULL,
    id_prvdor INT,
    CONSTRAINT fk_compra_sucursal FOREIGN KEY (id_scrsal) REFERENCES SUCURSAL(id_scrsal) ON DELETE CASCADE,
    CONSTRAINT fk_compra_proveedor FOREIGN KEY (id_prvdor) REFERENCES PROVEEDOR(id_prvdor) ON DELETE SET NULL
);
...
CREATE TABLE DETALLE_COMPRA (
    ...
    CONSTRAINT fk_detalle_compra_producto FOREIGN KEY (id_producto) REFERENCES PRODUCTO(id_producto) ON DELETE CASCADE,
...
CREATE TABLE RESERVA (
    ...
    CONSTRAINT fk_reserva_sucursal FOREIGN KEY (id_scrsal) REFERENCES SUCURSAL(id_scrsal)
);
...
CREATE TABLE PROMOCION (
    ...
    id_scrsal INT,
    CONSTRAINT fk_promocion_sucursal FOREIGN KEY (id_scrsal) REFERENCES SUCURSAL(id_scrsal)
);
